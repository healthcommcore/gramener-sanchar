...
function get_posts_from_otherlinks() {
    ...
    foreach ( $otherlinksdata as $data ) {
        ...
        $output .= ' <div class="card-header bg-transparent border-0 px-3 py-0" style="height: 65px;">';
        $output .= '<div class="display-3 mb-0 font-weight-bold"><span class="sm5 d-block">' . $data->post_title . '</span></div>';
        $output .= '</div>';
        ...
        $output .= '<a class="bg-color10 d-flex nav-link text-primary d-block justify-content-between font-weight-bold justify-content-center sm3 align-items-center cursor-pointer" href="' . $link . '" target="_blank" rel="noopener">';
        $output .= 'Visit Link <img src="' . get_template_directory_uri () . '/images/open-in-new-arrow.svg" alt="open-in-new-arrow">';
        $output .= '                  </a>';
        $output .= '</div>';
        $output .= '  </div>';
        $output .= '</div>';
